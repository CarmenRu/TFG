...
En este apartado se describen los requisitos mínimos necesarios para ejecutar InteraDD en la plataforma Render \cite{render}.
...
\begin{itemize}
    ...
    \item \textbf{Conexión a Internet:} necesaria para acceder a la versión desplegada de la aplicación.
\end{itemize}
...
No obstante, se recomienda ejecutar InteraDD de forma local. La versión desplegada en Render presenta las limitaciones inherentes al servicio gratuito utilizado, entre las que se incluyen periodos de inactividad tras un tiempo prolongado sin uso y restricciones en el tiempo máximo de ejecución de las peticiones. Como consecuencia, determinadas funcionalidades que requieren un mayor tiempo de procesamiento, como ocurre en la mayoría de las ejecuciones del botón \textit{Search for drug alternatives}, pueden no llegar a mostrar los resultados.

Con el fin de facilitar la ejecución local y garantizar la reproducibilidad del proyecto, el repositorio de GitHub, que contiene el código fuente, la documentación y el resto de los recursos necesarios para su compilación, instalación y ejecución, se encuentra en el Apéndice E, \textit{Manual del programador}. Concretamente en el apartado \textit{Compilación, instalación y ejecución del proyecto} se detallan tanto los requisitos previos como el procedimiento necesario para instalar y ejecutar la aplicación.
